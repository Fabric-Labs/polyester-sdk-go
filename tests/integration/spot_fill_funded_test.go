...
func TestSpotFill(t *testing.T) {
	...
	defer func() {
		...
		if hasMaker && makerOrderCreated {
			_, _ = maker.Orders.CancelAll(ctx, nil, nil, &symbol, nil, false, nil, nil)
		}
	}()
	...
	if !hasMaker {
		price, qty, err = testutil.BestAskParams(ctx, client, symbol, pair)
		if err != nil {
			if testutil.RouteUnavailable(err) {
				t.Skipf("orderbook unavailable for %s: %v", symbol, err)
			}
			if strings.Contains(err.Error(), "no visible asks") || strings.Contains(err.Error(), "below requested") {
				t.Skip(err.Error())
			}
			t.Fatal(err)
		}
		...
		if qty = strings.TrimSpace(os.Getenv("POLYESTER_TEST_TRADE_QTY")); qty == "" {
	...
	var makerBefore models.BalancesList
	var makerQuoteBefore, makerBaseBefore *big.Rat
	if hasMaker {
